...
(function () {
  ...
  const marginTips = {
    ...
    history: "Sign in once and every answer, quiz, and roadmap you generate is saved here automatically.",
  };
    ...
    if (currentUser) {
      accountArea.innerHTML = `
        <div class="account-chip">
          <span>Signed in as <span class="account-chip__name">${escapeHtml(currentUser.username)}</span></span>
          <button class="btn btn--ghost" id="btnLogout" type="button">Sign out</button>
        </div>`;
      document.getElementById("btnLogout").addEventListener("click", handleLogout);
    } else {
      accountArea.innerHTML = `<button class="btn btn--ghost" id="btnShowAuth" type="button">Sign in</button>`;
  ...
  document.getElementById("btnHistorySignIn")?.addEventListener("click", () => openAuthModal("login"));
  ...
  async function loadHistory() {
    ...
    try {
      ...
      if (!data.items.length) {
        listEl.innerHTML = `<p class="history-empty">Nothing saved yet — try one of the tools above.</p>`;
        return;
      }
      ...
      data.items.forEach((item) => {
        ...
        row.innerHTML = `
          <span class="history-item__feature">${featureLabels[item.feature] || item.feature}</span>
          <span class="history-item__text"></span>
          <span class="history-item__time">${when.toLocaleDateString()} ${when.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
        `;
        row.querySelector(".history-item__text").textContent = item.input_text;
        listEl.appendChild(row);
      });
    } catch (err) {
      listEl.innerHTML = `<p class="history-empty">${err.message}</p>`;
    }
  }
