// EduGenie frontend logic — auth gate, tabs, API calls, history, result rendering.
(function () {
  "use strict";
  ...
  const authPanels = document.querySelectorAll(".auth__panel");
  ...
  authTabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      authPanels.forEach((p) => {
        const active = p.dataset.authpanel === target;
  ...
  document.getElementById("form-login").addEventListener("submit", async (e) => {
    ...
    try {
      const user = await callApi("/api/auth/login", { email, password });
      showApp(user);
    } catch (err) {
  ...
  document.getElementById("form-signup").addEventListener("submit", async (e) => {
    ...
    try {
      const user = await callApi("/api/auth/signup", { name, email, password });
      showApp(user);
    } catch (err) {
      errEl.textContent = err.message;
      errEl.hidden = false;
  ...
  async function checkHealth() {
    ...
    try {
      ...
      if (data.gemini_configured) {
        ...
        dot.title = "Gemini connected";
      } else if (data.local_fallback_enabled) {
        dot.className = "status-dot warn";
        dot.title = "Using local fallback model";
      } else {
        dot.className = "status-dot error";
        dot.title = "No AI backend configured";
      }
    } catch {
      dot.className = "status-dot error";
  ...
  async function loadHistory() {
    ...
    try {
      const items = await callApi("/api/history");
      if (!items.length) {
        list.innerHTML = '<p class="history-empty">Nothing here yet — your past questions and results will show up after you use a tool.</p>';
        return;
      }
      list.innerHTML = "";
      ...
      items.forEach((item) => {
        const node = tpl.content.cloneNode(true);
        node.querySelector(".history-item__tool").textContent = toolLabels[item.tool] || item.tool;
        node.querySelector(".history-item__date").textContent = new Date(item.created_at).toLocaleString();
        node.querySelector(".history-item__input").textContent = item.input_summary;
        node.querySelector(".history-item__result").textContent = item.result;
        const article = node.querySelector(".history-item");
        node.querySelector(".history-item__delete").addEventListener("click", async () => {
          try {
            await callApi(`/api/history/${item.id}`, null, "DELETE");
            article.remove();
          } catch {
            /* ignore */
          }
      ...
      list.innerHTML = `<p class="history-empty">Couldn't load history: ${err.message}</p>`;
    }
  }
})();
